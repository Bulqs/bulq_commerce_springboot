...
public interface BusinessRepository extends JpaRepository<Business, Long> {
       ...
       "OR (:business_name IS NULL OR b.business_name LIKE CONCAT('%', :business_name, '%')) " +  // Filter by business name
       "OR (:email IS NULL OR b.email LIKE CONCAT('%', :email, '%')) " +  // Filter by email
       "OR (:business_type IS NULL OR b.business_type LIKE CONCAT('%', :business_type, '%')) " +  // Filter by business type
       "OR (:phone_number IS NULL OR b.phone_number LIKE CONCAT('%', :phone_number, '%'))")  // Filter by phone number
